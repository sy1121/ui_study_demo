...
public class LeafLoadingView extends View{
    ...
    //淡白色
    private static final int   WHITE_COLOR = 0Xfffa800;
    //橙色
    ...
    private void drawProgressAndLeafs(Canvas canvas){
       /* if(mProgress >= TOTAL_PROGRESS){
        ...
        //mProgessWidth为进度条的宽度， 根据档期那进度算出进度条的位置
    ...
    private void drawLeafs(Canvas canvas){
        ...
        for(int i = 0; i < mLeafInfos.size(); i++){
            ...
            if(currentTime > leaf.startTime && leaf.startTime != 0){
                ...
                //通过时间关联旋转角度，则可以直接通过修改LEAF_TPTATE_TIME调节叶子旋转快慢
    ...
    private int getLocationY(Leaf leaf){
        ...
        float a = mMiddelAmplitude;
        switch (leaf.type){
            case LITTLE:
                // 小振幅 = 中等振幅 - 振幅差
                a = mMiddelAmplitude - mAmplitudeDisparity;
                break;
            case MIDDLE:
                a = mMiddelAmplitude;
                break;
    ...
    private class LeafFactory{
        ...
        public Leaf generateLeaf() {
            ...
            //为了产生交错的刚觉， 上开始的时间有一定的随机性
            mLeafFloatTime = mLeafRotateTime <= 0 ? LEAF_ROTATE_TIME : mLeafRotateTime;
    ...
    public void setMiddleAmplitude(int amplitude) {
        this.mMiddelAmplitude = amplitude;
    }
