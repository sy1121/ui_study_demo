...
public class RemoteControlView extends View{
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        pts[0] = event.getX();
        pts[1] = event.getY();
        Log.i(TAG,"before pts = (" + pts[0]+","+pts[1]+")");
        mMapMatrix.mapPoints(pts);
        Log.i(TAG,"after pts = (" + pts[0]+","+pts[1]+")");
        int x = (int)pts[0];
        int y = (int)pts[1];
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.i(TAG,"before mMapMatrix = " + mMapMatrix.toShortString());
        // 获取测量矩阵(逆矩阵)
        if (mMapMatrix.isIdentity()) {
            canvas.getMatrix().invert(mMapMatrix);
        }
        Log.i(TAG,"after mMapMatrix = " + mMapMatrix.toShortString());
        // 绘制默认颜色
